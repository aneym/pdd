...
- When the purpose is amended, what gets built next is re-ranked — purpose stays upstream of
  the work, never drifting behind it.
...
The core skill is a markdown file agents read — no build step.
...
(a grill to shared understanding, not a form to fill), **a purpose tree instead of a linear
spec chain**, and **work derived from purpose rather than task decomposition** — with the
presentation law keeping the human's screen honest the whole way.

## Open (figure-out-through-building)

- **A visual walkthrough surface** — a bundled HTML page (open items on top, ramble box,
  synthesis below) so owners can read everything and talk against it. Prototype exists in a
